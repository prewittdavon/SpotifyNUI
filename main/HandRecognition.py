...
# **********************************************
# * Hand Gesture Recognition Implementation v2.0
# * 13 August 2016
# * Davon Prewitt, Mahaveer Verma
# **********************************************
...
def mark_hand_center(frame_in,cont):    
    ...
    if(max_d>radius_thresh*frame_in.shape[1]):
        thresh_score=True
        cv2.circle(frame_in,pt,int(max_d),(255,255,255),2)
    else:
        thresh_score=False
...
while(1):
    ...
    cv2.putText(frame,"Spotify NUI",(int(20),int(20)),cv2.FONT_HERSHEY_SIMPLEX,1,(106,227,104),4,8)
    cv2.rectangle(frame,(int(cap_region_x_begin*frame.shape[1]),0),(frame.shape[1],int(cap_region_y_end*frame.shape[0])),(106,227,104),4)
    ...
    if (not (capture_done and bg_captured)):
        if(not bg_captured):
            cv2.putText(frame,"Remove hand from the frame and press 'b' to capture background",(int(0.05*frame.shape[1]),int(0.97*frame.shape[0])),cv2.FONT_HERSHEY_SIMPLEX,0.5,(106,227,104),1,8)
        else:
            cv2.putText(frame,"Place hand inside boxes and press 'c' to capture hand histogram",(int(0.08*frame.shape[1]),int(0.97*frame.shape[0])),cv2.FONT_HERSHEY_SIMPLEX,0.5,(106,227,104),1,8)
        first_iteration=True
        finger_ct_history=[0,0]
        ...
        for i in range(capture_box_count):
            cv2.rectangle(frame,(box_pos_x[i],box_pos_y[i]),(box_pos_x[i]+capture_box_dim,box_pos_y[i]+capture_box_dim),(106,227,104),4)
    else:
